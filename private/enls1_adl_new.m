...
numobs=length(y);
    ...
    if size(x.xmidas,3)>1
        exp0= repmat(exp0,size(xmidas,3),1);
    end
...
a00=[a00(1:2); exp0; a00(3:end)] ; % First row: b_0, b_1; Second row: theta_1, theta_2; third : b_3, y_l];
options0=optimset('Display','iter-detailed','TolFun',1e-8,'TolX',1e-8,'MaxIter',1000,'MaxFunEvals',1000,'Jacobian','on');
...
a=aopt;
fprintf(" The difference from initial input... \n");
[aopt, a00]
[errors,jac,weights]=mfrvobj_adl(aopt,x,y,polyconstr);
HH=hessian('ssr_mfrvobj_adl',a,x,y,polyconstr);
warning('Suspended NAN notification of the Hessian matrix! \n')
%if rcond(HH)<eps
%    invHESSIAN=HH*NaN;
%else
    invHESSIAN=inv(HH);
...
if isequal(output_type,'struct')      % Regressor coefficients
    if size(x.xmidas,3)<2
        weightsNN=weights*aopt(2);
    else
        pted_theta = 2+length(exp0)+1;
        HiFreqBeta_no = 2+length(exp0)+size(x.xmidas,3)-1;
        beta_x = aopt([2,pted_theta:HiFreqBeta_no]);
        weightsNN=weights.*beta_x';                 % Be aware that weights differ for each regressor
    end
        
    aoptxols = aopt(end-size(x.xols,2)+1:end);      % Lag coefficients
    aoptmidas = aopt(1:end-size(x.xols,2));         % Exclude lag coefficients
